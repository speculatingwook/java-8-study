...
public class SongService {
    ...
    public List<Song> getSongsOverDuration(int threshold) {
        return songs.stream()
                .filter(song -> song.getDuration() >= threshold)
                .collect(Collectors.toList());
    }
    ...
    public boolean areAllSongsAboveDuration(int threshold) {
        return songs.stream()
                .allMatch(song -> song.getDuration() >= threshold);
    }
        ...
        return songs.stream()
                .collect(Collectors.groupingBy(
                        ...
                        Collectors.collectingAndThen(
                                Collectors.maxBy(Comparator.comparingInt(Song::getDuration)),
                                Optional::get
                        )
                ));
    }
    ...
    public Optional<Song> getSongWithLongestTitle() {
        return songs.stream()
                .max(Comparator.comparingInt(song -> song.getTitle().length()));
    }
    ...
    public List<String> getUpperCaseTitlesOfSongsAboveDuration(int threshold) {
        return songs.stream()
                .filter(song -> song.getDuration() >= threshold)
                .map(song -> song.getTitle().toUpperCase())
                .collect(Collectors.toList());
    }
    ...
    public Map<String, Double> getAverageDurationByGenre() {
        return songs.stream()
                .collect(Collectors.groupingBy(
                        Song::getGenre,
                        Collectors.averagingInt(Song::getDuration)
                ));
    }
    ...
    public List<Map.Entry<String, Double>> getGenresSortedByAverageDuration() {
        ...
        return avgDurationMap.entrySet().stream()
                .sorted((e1, e2) -> {
                    // 먼저 평균 재생 시간을 내림차순으로 비교
                    int cmp = Double.compare(e2.getValue(), e1.getValue());
                    if (cmp == 0) {
                        // 평균이 같다면 장르명을 오름차순으로 비교
